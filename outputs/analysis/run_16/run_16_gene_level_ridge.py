"""
BioEval Run 16 -- Gene-Level Feature Ridge for K562/RPE1
Resolves one-hot LOO degeneracy (C1) by using perturbation PCA profiles as features.

Strategy:
- K562/RPE1 have only single-KO perturbations ("GENE+ctrl")
- One-hot features make LOO degenerate (removes only distinguishing feature)
- Solution: compute PCA on all cells, use each perturbation's mean PCA embedding
  as features. LOO now removes perturbation i but other perturbations' PCA
  profiles remain informative.
- Also add: perturbation variance in PCA space, cell count (log),
  mean expression of KO-perturbed genes
"""
...
OUTPUT_DIR = r"C:\test\llm_project\nexus-science-win\outputs\analysis\run_16"
...
def load_and_prepare(adata, ct_name):
    ...
    return {'cell_type': ct_name, 'perturbations': perts_sorted, 'true_logfc': true_logfc,
            'n_perts': len(perts_sorted), 'n_genes': true_logfc.shape[1], 'adata': adata,
            'pert_col': pert_col, 'ctrl_key': ctrl_key}
...
def build_gene_level_features(ds_data, n_pca=30):
    """Build PCA-based features for K562/RPE1 single-KO datasets.

    For each perturbation, feature = mean PCA embedding of its cells.
    This avoids one-hot LOO degeneracy because removing perturbation i
    still leaves informative PCA profiles from other perturbations.
    """
    adata = ds_data['adata']
    pert_col = ds_data['pert_col']
    ctrl_key = ds_data['ctrl_key']
    perts = ds_data['perturbations']
    n_perts = ds_data['n_perts']

    # Step 1: PCA on control cells
    ...
    pca.fit(ctrl_data)
    log(f"    PCA explained var (cumul, {n_pca} PCs): {pca.explained_variance_ratio_.cumsum()[-1]:.3f}")

    # Step 2: Compute PCA features for each perturbation
    pca_means = np.zeros((n_perts, n_pca), dtype=np.float32)
    pca_vars = np.zeros((n_perts, n_pca), dtype=np.float32)
    cell_counts = np.zeros(n_perts, dtype=np.float32)

    for i, pert in enumerate(perts):
        mask = adata.obs[pert_col] == pert
        pdata = adata[mask].X
        if hasattr(pdata, "toarray"):
            pdata = pdata.toarray()
        pdata = np.asarray(pdata, dtype=np.float32)
        projected = pca.transform(pdata)
        pca_means[i] = projected.mean(axis=0)
        pca_vars[i] = projected.var(axis=0)
        cell_counts[i] = np.log1p(mask.sum())

    # Step 3: Combine features [pca_mean | pca_var | log_cell_count]
    # Use pca_mean as primary features (most informative)
    # pca_var and cell_count as auxiliary
    X = np.hstack([pca_means, pca_vars, cell_counts[:, None]])
    log(f"    Feature matrix: {X.shape} (pca_mean={n_pca} + pca_var={n_pca} + log_count=1)")

    # Check: is feature matrix rank sufficient for LOO?
    rank = np.linalg.matrix_rank(X)
    log(f"    Feature rank: {rank}/{X.shape[1]} (n_perts={n_perts})")

    return X, {'n_pca': n_pca, 'feature_dim': X.shape[1], 'feature_rank': rank,
               'pca_explained_var_cumul': float(pca.explained_variance_ratio_.cumsum()[-1])}

# ============================================================
# Ridge LOO (analytical) + models
# ============================================================
...
def train_real_models(ds_data, use_gene_features=True, n_pca=30):
    true_logfc = ds_data['true_logfc']
    n_perts, n_genes = true_logfc.shape
    perts = ds_data['perturbations']

    if use_gene_features:
        X, feat_info = build_gene_level_features(ds_data, n_pca=n_pca)
        ftype = "gene_pca"
    else:
        # Fallback: additive features (works for Norman, fails for K562/RPE1)
        def parse_pert(name):
        ...
        feat_info = {'feature_type': ftype, 'feature_dim': X.shape[1]}
    ...
    # 5-6. Degraded variants
    rp = model_predictions['ridge']
    gs = true_logfc.std(axis=0)
    ns = 0.15 * np.median(gs[gs > 0]) if (gs > 0).any() else 0.01
    ...
    # 8-9. Oracle baselines (excluded from H3)
    model_predictions['constant_shrink'] = true_logfc * 0.15
    model_metadata['constant_shrink'] = {'type': 'oracle', 'signal_frac': 0.15}
    model_predictions['half_signal'] = true_logfc * 0.5 + np.random.normal(0, ns*0.5, true_logfc.shape)
    model_metadata['half_signal'] = {'type': 'oracle', 'signal_frac': 0.5}

    return model_predictions, model_metadata, feat_info

# ============================================================
# Full Evaluation Pipeline
# ============================================================

def evaluate_dataset(ct_name, ds_data, use_gene_features, n_pca=30):
    true_logfc = ds_data['true_logfc']
    preds, meta, feat_info = train_real_models(ds_data, use_gene_features=use_gene_features, n_pca=n_pca)
    model_names = list(preds.keys())
    log(f"  Models ({len(model_names)}): {model_names}")

    # Quick check: is Ridge degenerate?
    ridge_r2 = r2_score(true_logfc.flatten(), preds['ridge'].flatten())
    mp_r2 = r2_score(true_logfc.flatten(), preds['mean_predictor'].flatten())
    log(f"  Ridge R2={ridge_r2:.4f}, mean_predictor R2={mp_r2:.4f}, diff={ridge_r2-mp_r2:.4f}")
    if abs(ridge_r2 - mp_r2) < 0.01:
        log(f"  WARNING: Ridge ≈ mean_predictor (degenerate)")
    ...
    # H3 (corrected: exclude oracle baselines)
    log(f"\n  H3: Trained vs Baseline (corrected)...")
    true_baselines = ['mean_predictor', 'mean_effect']
    trained = [m for m in model_names if meta[m]['type'] == 'trained']
    h3r = {}
    for metric in ['Dir_deg','Dir_all','DEG_auprc','R2','f1@50','dir_discovery_deg']:
        # Get values from correct source (eval_m vs ds_results)
        if metric in ['f1@50','f1@100','dir_discovery_deg']:
            ts = [ds_results[m][metric] for m in trained]
            bs_vals = [ds_results[m][metric] for m in true_baselines]
        else:
            ts = [eval_m[m][metric] for m in trained]
            bs_vals = [eval_m[m][metric] for m in true_baselines]

        bt = max(ts) if ts else 0; bb = max(bs_vals) if bs_vals else 0
    ...
    # Clean up adata reference to save memory
    ds_data_copy = {k: v for k, v in ds_data.items() if k != 'adata'}

    return {
        'n_perts': ds_data['n_perts'], 'n_genes': ds_data['n_genes'],
        'feature_info': feat_info, 'ridge_r2': float(ridge_r2), 'mp_r2': float(mp_r2),
...
def main():
    t0 = time.time()
    log("=" * 60)
    log("BioEval Run 16 -- Gene-Level Feature Ridge for K562/RPE1")
    log("=" * 60)
    ...
    # K562 and RPE1: gene-level PCA features
    for ct_name in ["K562", "RPE1"]:
        if ct_name not in datasets: continue
        ds_data = datasets[ct_name]
        log(f"\n{'='*60}")
        log(f"ANALYZING: {ct_name} ({ds_data['n_perts']} perts, {ds_data['n_genes']} genes) -- GENE PCA FEATURES")
        log(f"{'='*60}")
        all_results[ct_name] = evaluate_dataset(ct_name, ds_data, use_gene_features=True, n_pca=30)
        # Free adata memory
        del datasets[ct_name]['adata']

    # Norman: additive features (already works, re-verify)
    if "Norman" in datasets:
        ct_name = "Norman"
        ds_data = datasets[ct_name]
        log(f"\n{'='*60}")
        log(f"ANALYZING: {ct_name} ({ds_data['n_perts']} perts, {ds_data['n_genes']} genes) -- ADDITIVE FEATURES")
        log(f"{'='*60}")
        all_results[ct_name] = evaluate_dataset(ct_name, ds_data, use_gene_features=False)
        del datasets[ct_name]['adata']
    ...
    for ct, r in all_results.items():
        revs = [x for x in r['reversal_analysis'] if x['interp'] in ('REVERSAL', 'PARTIAL')]
        log(f"  {ct}: Ridge R2={r['ridge_r2']:.4f} (mp={r['mp_r2']:.4f}), "
            f"H1_reversals={len(revs)}/{len(r['reversal_analysis'])}, H2={r['h2_pass_rate']:.1%}")
        if r['ridge_r2'] - r['mp_r2'] < 0.01:
            log(f"    WARNING: DEGENERATE -- Ridge ≈ mean_predictor")
        for metric, h3 in r['h3_results'].items():
            log(f"    H3 {metric}: trained_wins={h3['trained_wins']}")

    # Comparison with run_15
    log(f"\n{'='*60}")
    log("COMPARISON: run_15 (one-hot) vs run_16 (gene PCA)")
    log(f"{'='*60}")
    for ct in ["K562", "RPE1"]:
        if ct in all_results:
            r = all_results[ct]
            log(f"  {ct}: Ridge R2={r['ridge_r2']:.4f} (run_15 was -0.027)")
            log(f"    Feature type: {r['feature_info'].get('n_pca', 'additive')}")

    # Save
    elapsed = time.time() - t0
    final = {'run':'run_16','timestamp':time.strftime('%Y-%m-%d %H:%M:%S'),
             'runtime_s':elapsed,'model_type':'real_trained_ridge_gene_features',
             'datasets':all_results}
    with open(os.path.join(OUTPUT_DIR, "run_16_results.json"), 'w') as f:
